...
BasicChannelHandler::BasicChannelHandler(BaseReliableObj *parent, appInt8 ifcLoc, RemoteAddr &rmadr, appInt8 ifcSrc, appInt8 ifcDst) :
        ChannelHandler(parent, ifcLoc, rmadr, ifcSrc, ifcDst), senderWindow(1), cwnd(1.),
        duplicateCount(0), ssthresh(INITIAL_SSTHRESH), recover(0), rtt(0), srtt(0), rto(TIMER_SECOND_IN_US), rttvar(0), G(TIMER_GRANULITY_US),
        alpha(0.125), beta(0.25), packetLock(), status(SLOW_START), closed(FALSE), deadChannel(FALSE), started(FALSE)
{
    sendCSNPacket();
}
...
appInt BasicChannelHandler::pktRecv(Packet* pkt) {
    if(!recieverBuff.recvStarted and !(pkt->header.flag&FLAG_PSH))
        return 0;
    if(pkt->header.flag&FLAG_PSH)
...
void BasicChannelHandler::sendSeq(appInt16 seq) {
    sendSeqLock.lock();
...
//    APP_ASSERT(ifcSch)
    for(auto i = 0; i < pktCnt; i++, nextPkt++){
        ...
        self->parent->sendPacketTo(0, pkt, NULL, 0);
    }
    return NULL;
}
...
void BasicChannelHandler::sendCSNPacket() {
    ...
    pkt->header.flag = FLAG_DAT|FLAG_CSN;
